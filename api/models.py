...
class Profile(models.Model):
    ...
    # virtual account generation tracking
    account_generation_failed = models.BooleanField(default=False)
    ...
    def save(self, *args, **kwargs):
        if self.code == '':
            ...
            base = self.user.username[:6].upper()
...
class Transaction(models.Model):
    ...
    network = models.CharField(max_length=300,default='MTN',blank=True,null=True)
...
class PushNotification(models.Model):
    ...
    recipient_user = models.ForeignKey(
        ...
        help_text='Required only when recipient type is "Specific User"',
    )
    ...
    def __str__(self):
        return f'{self.title} → {self.get_recipient_type_display()} ({self.sent_at:%Y-%m-%d %H:%M})'
