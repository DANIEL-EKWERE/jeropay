...
#}
            ...
            if response.status_code == 200:
                ...
                provider_status = response_data.get("status", "").lower()
                ...
                # If provider returned a failed status, refund immediately
                if provider_status == 'failed':
                    ...
                    Transaction.objects.create(
                        ...
                        old_balance=balances[0],
                        new_balance=balances[0],  # balance restored
                        phone_number=phone_number,
                        status='Refunded',
                        amount=amount,
                        type='Airtime',
                    )
                    _send_push(request.user, 'Airtime Purchase Failed', f'₦{amount} has been refunded to your wallet.')
                    return Response({'status': 'error', 'message': 'Purchase failed. Your wallet has been refunded.'}, status=400)
                ...
                Wallet.objects.filter(user=profile).update(total_purchase=wallet.total_purchase + amount)

                _send_push(request.user, 'Airtime Purchase Successful', f'You purchased ₦{amount} airtime on {network}')

                return Response({'status': 'success', 'data': TransactionSerializer(trans).data}, status=200)

            return Response({'status': 'error', 'message': response.json()}, status=400)
        return Response(
            data={
                'status': 'error',
...
#}
            ...
            try:
                ...
                if response.status_code == 201 or response.status_code == 200:
                    ...
                    data_response = response_data.get("response", "")
                    request_id = response_data.get("request-id", "")
                    provider_status = (response_data.get("status") or response_data.get("Status") or "").lower()

                    if provider_status == 'failed' or provider_status == 'fail':
                        return Response(
                            data={
                                'status': 'error',
                                'message': 'There was an issue processing this request. No charge was made.',
                            },
                            status=400,
                        )
                    ...
                    wallet = Wallet.objects.get(user=profile)
                    Wallet.objects.filter(user=profile).update(total_purchase=wallet.total_purchase + data_plan_price)

                    _send_push(request.user, 'Data Purchase Successful', f'You purchased {data_bandwidth} data on {data_network}')

                    return Response(
                        data={'status': 'success', 'message': TransactionSerializer(trans).data},
                        status=200,
                    )

                else:
                    return Response(
                        data={'status': 'error', 'message': response.json()},
                        status=400,
                    )
            except requests.exceptions.Timeout:
                logger.error('Request to KVDATA API Timed out')
...
class BillPurchaseMixin(RequestWrapper, WalletCheckMixin):
    def post(self, request, *args, **kwargs):
        ...
        if  serializer.is_valid():
            ...
            _send_push(request.user, 'Transaction Successful', f'Your ₦{amount} transaction was completed successfully.')
